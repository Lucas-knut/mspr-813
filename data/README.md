# Données du Projet - Architecture Medallion

## 📂 Structure Bronze/Silver/Gold

```
data/
├── bronze/   🥉 Données brutes sources (CSV, XLSX) - Immuables
├── silver/   🥈 Données nettoyées (Parquet) - Validées et typées
└── gold/     🥇 Résultats ML et agrégations - Business-ready
```

> **📖 Documentation complète** : [`../docs/DATA_ARCHITECTURE.md`](../docs/DATA_ARCHITECTURE.md)
...
1. **Élections agrégées 1999-2024** (2.2 GB)
   - Source : data.gouv.fr
   - Placer dans `data/bronze/elections_agregees_1999_2024.csv`

2. **Revenus par commune** (4.8 MB)
   - Source : data.gouv.fr
   - Placer dans `data/bronze/revenus_commune.csv`

3. **Référentiel communes** (3 MB)
   - Source : INSEE
   - Placer dans `data/bronze/referentiel_communes.csv`

4. **Population historique** (40 MB, ZIP)
   - Source : INSEE
   - Extraire dans `data/bronze/population_historique_1968_2022/`

5. **Diplômes et formation** (81 MB, ZIP)
   - Source : INSEE
   - Extraire dans `data/bronze/diplomes_formation_2022/`

6. **CSP actifs 25-54 ans** (28.5 MB, ZIP)
   - Source : INSEE
   - Extraire dans `data/bronze/csp_actifs_2554/`

## 📊 Formats par Couche

- **Bronze (`bronze/`)** : CSV, Excel (XLSX) - Format original
- **Silver (`silver/`)** : Parquet (compression snappy) - Optimisé pour lecture
- **Gold (`gold/`)** : Parquet + JSON (métriques) + CSV (exports)
